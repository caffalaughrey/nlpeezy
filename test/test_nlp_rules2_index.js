...
const fs = require('fs');
const initialMutation = require('../src/nlp/rules2/ga/initial-mutation');
const lex = require('../src/nlp/rules2/lex');
...
const RuleProvider = rules2.RuleProvider;
...
let getDefaultRuleSettings = () => {
	let settingsPath =
		path.join(appDir, 'src', 'nlp', 'rules2', 'settings.default.json');
...
let getRuleSettings = language => {
	let settingsPath =
		path.join(appDir, 'src', 'nlp', 'rules2', language, 'settings.json');

		return JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
}

describe('/nlp/rules2/index', () => {
	describe('#applyTo()', () => {
    it('applies default rules during linear parse phase', done => {
      let parent = new UnprocessedToken('(lúibíní!)');
      let tokens = rules2.applyTo(parent, parsePhases.LINEAR);
      ...
      assert(tokens[2].getValue() == '!', INVALID_VALUE);
		...
		it('applies Irish language eclipsis info during linear parse phase', done => {
