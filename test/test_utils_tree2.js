...
const Tree = require('../src/utils/tree2').Tree;
...
describe('/utils/tree2', () => {
    ...
    it('knows its filtered, mapped nodes from a tree of three levels', done => {
      ...
      let output = tree.filterMap(filterFn);

      assert(output.join(' ') == 'a aa ba ca', INVALID_OUTPUT);
      done();
    });

    it('knows its descendant nodes unwound from inner nodes', done => {
      let addSkip = true;
      let lastIndex = 0;
      ...
      let output = tree.unwind(InnerNode, transformFn);
